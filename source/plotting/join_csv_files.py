    ...
    for algo in tqdm(algos, desc=f"{env}"):
        for ds in datasets:
            for folder in folders:
                ...
                np.savetxt(os.path.join(target, folder, env, f"{algo}_{ds}.csv"), results, delimiter=";")

                # online
                results = []
                for run in range(runs):
                    try:
                        results.append(np.genfromtxt(os.path.join(origin, folder, f"ex{e + 1}",
                                                                  f"{env}_online_run{run + 1}.csv")).tolist())
                    except:
                        pass
                length = len(results)
                results = np.asarray(results).reshape(length, -1).transpose((1, 0))
